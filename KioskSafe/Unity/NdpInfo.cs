...
namespace KioskSafe.Unity
{
    ...
    public sealed class NdpInfo
    {
        ...
        /// 源码来自：https://docs.microsoft.com/zh-cn/dotnet/framework/migration-guide/how-to-determine-which-versions-are-installed
        /// </summary>
        /// <returns>
        /// 一个字典，key 为 .NET Framework 的可共存版本，value 为此共存版本下就地更新（不可共存）的最新版本。
        /// <para>形如：</para>
        ...
        public static Dictionary<string, NdpInfo> ReadFromRegistry()
        {
            ...
            {
                var ndpKey = localMachine.OpenSubKey(@"SOFTWARE\Microsoft\NET Framework Setup\NDP\");
